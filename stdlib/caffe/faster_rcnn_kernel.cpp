...
public:
  FasterRCNNKernel(const Kernel::Config& config)
    : CaffeKernel(config) {}
  ...
  void net_config() override {
    ...
    f32* blob_data = device_.type == DeviceType::GPU
      ? blob->mutable_gpu_data()
      : blob->mutable_cpu_data();
    memcpy_buffer((u8*) blob_data, device_,
                  (u8*) buf, CPU_DEVICE,
                  3*sizeof(f32));
  }
};
...
REGISTER_KERNEL(FasterRCNN, FasterRCNNKernel).device(DeviceType::CPU).num_devices(1);
REGISTER_KERNEL(FasterRCNN, FasterRCNNKernel).device(DeviceType::GPU).num_devices(1);
