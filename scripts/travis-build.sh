...
build_docker() {
    ...
    then
        ...
        docker run $DOCKER_REPO:$1-local /bin/bash \
               -c "adduser --disabled-password --gecos \"\" user && su -c \"cd /opt/scanner/dist && pip3 uninstall protobuf && (yes | pip3 install --user *) && cd /opt/scanner/build && CTEST_OUTPUT_ON_FAILURE=1 make test\" user"
