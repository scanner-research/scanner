...
build_docker() {
    ...
    then
        ...
        docker run $DOCKER_REPO:$1-local /bin/bash \
               -c "adduser --disabled-password --gecos \"\" user && ls -lah /usr/local/lib && ls -lah /usr/local && su -c \"cd /opt/scanner/dist && (yes | pip3 install --user *) && cd /opt/scanner/build && CTEST_OUTPUT_ON_FAILURE=1 make test ARGS='-V'\" user"
