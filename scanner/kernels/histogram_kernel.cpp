...
class HistogramKernelCPU : public FrameKernel {
public:
  HistogramKernelCPU(const Kernel::Config& config)
    : FrameKernel(config), config_(config) {}

  void execute(const BatchedColumns& input_columns,
               BatchedColumns& output_columns) override {
    std::vector<proto::Frame> frames = get_frames(input_columns[0]);

    size_t hist_size = BINS * 3 * sizeof(float);
    i32 input_count = frames.size();
    u8* output_block = new_block_buffer(config_.devices[0],
                                        hist_size * input_count,
                                        input_count);

    cv::Mat tmp;
    for (i32 i = 0; i < input_count; ++i) {
      cv::Mat img(frame_height_, frame_width_, CV_8UC3,
                  (u8*) frames[i].buffer());
...
class HistogramKernelGPU : public FrameKernel {
public:
  ...
  void new_frame_size() override {
    streams_.resize(0);
    streams_.resize(num_cuda_streams_);
    planes_.clear();
    for (i32 i = 0; i < 3; ++i) {
      planes_.push_back(
          cvc::GpuMat(frame_height_, frame_width_, CV_8UC1));
    }
  }
