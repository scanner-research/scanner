...
void CaffeInputKernel::transform_halide(u8* input_buffer,
  ...
  auto func = device_.type == DeviceType::GPU
  ...
  int error = func(&input_buf, frame_width, frame_height,
                   net_input_width_, net_input_height_, descriptor.normalize(),
                   descriptor.mean_colors(2), descriptor.mean_colors(1),
                   descriptor.mean_colors(0), &output_buf);
...
void CaffeInputKernel::execute(const BatchedColumns& input_columns,
  ...
  u8* output_block = new_block_buffer(
    device_, net_input_size * input_count, input_count);
...
REGISTER_KERNEL(CaffeInput, CaffeInputKernel)
    .device(DeviceType::GPU).num_devices(1);
}
