...
#ifndef HAVE_CUDA
void CameraMotionEvaluator::from_homography(std::vector<cv::Mat> &inputs,
  ...
  cv::Matx33d K(fx, 0, mx, 0, fy, my, 0, 0, 1);
...
void CameraMotionEvaluator::from_background_subtraction(
    std::vector<cv::Mat> &inputs, std::vector<u8 *> &output_buffers,
    std::vector<size_t> &output_sizes) {
...
void CameraMotionEvaluator::evaluate(std::vector<Mat> &inputs,
                                     std::vector<u8 *> &output_buffers,
                                     std::vector<size_t> &output_sizes) {
#ifdef HAVE_CUDA
