...
FaceEvaluator::FaceEvaluator() {
  ...
  // if
  // (!face_detector.load("/opt/stanford/wcrichto/haarcascade_frontalface_alt.xml"))
  // {
  if (!face_detector.load(
          "/home/wcrichto/scanner/haarcascade_frontalface_alt.xml")) {
...
void hh_find_faces(cv::Mat &img, std::vector<cv::Rect> &faces) {
  ...
  objects_detection::detections_t detections =
      objects_detection::get_detections();
  ...
  for (auto &det : detections) {
    ...
    cv::Rect face = cv::Rect(min_corner.x(), min_corner.y(),
...
void FaceEvaluator::evaluate(std::vector<Mat> &inputs,
