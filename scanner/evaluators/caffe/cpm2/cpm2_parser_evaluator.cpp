...
int CPM2ParserEvaluator::connect_limbs(
    std::vector<std::vector<double>> &subset,
    std::vector<std::vector<std::vector<double>>> &connection,
    const float *heatmap_pointer, const float *peaks, float *joints) {
  /* Parts Connection ---------------------------------------*/
  // limbSeq = [15 2; 2 1; 2 3; 3 4; 4 5; 2 6; 6 7; 7 8; 15 12; 12 13; 13 14; 15
  // 9; 9 10; 10 11];
  ...
  for (int k = 0; k < num_limb_seq; k++) {
    ...
    // 	    cout << endl;
    // 	}
    // 	//cout << "debug" << score_mid[ 216 * INIT_PERSON_NET_WIDTH + 184] <<
    // endl;
    // }
      ...
      for (int j = 0; j < NUM_PARTS; j++) {
        ...
        if (idx) {
          ...
          joints[cnt * NUM_PARTS * 3 + j * 3 + 1] =
              peaks[idx - 1] * metadata_.height() / (float)net_input_height_;
          joints[cnt * NUM_PARTS * 3 + j * 3] =
              peaks[idx - 2] * metadata_.width() / (float)net_input_width_;
        } else {
