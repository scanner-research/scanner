...
void FacenetCPUInputTransformer::configure(
...
void FacenetCPUInputTransformer::transform_input(
  u8* input_buffer,
  f32* net_input,
  int batch_size)
{
  ...
  for (i32 i = 0; i < batch_size; ++i) {
    ...
    for (i32 r = 0; r < planar_input.rows; ++r) {
      ...
      u8* input_pos = reinterpret_cast<u8*>(
        net_input +
        i * (net_input_width_ * net_input_height_ * 3) +
        r * net_input_height_);
