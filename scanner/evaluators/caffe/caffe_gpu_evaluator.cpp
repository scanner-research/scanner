...
CaffeGPUEvaluator::CaffeGPUEvaluator(EvaluatorConfig config,
                                     NetDescriptor descriptor, int device_id)
    : CaffeEvaluator(),
  ...
  cv::Mat cpu_mean_mat(descriptor.mean_height * 3, descriptor.mean_width,
  ...
  for (size_t i = 0; i < NUM_CUDA_STREAMS; ++i) {
    input_mats.push_back(
        cv::cuda::GpuMat(args.metadata[0].height + args.metadata[0].height / 2,
                         args.metadata[0].width, CV_8UC1));
  }

  std::vector<cv::cuda::GpuMat> rgba_mat;
  for (size_t i = 0; i < NUM_CUDA_STREAMS; ++i) {
    rgba_mat.push_back(cv::cuda::GpuMat(args.metadata[0].height,
                                        args.metadata[0].width, CV_8UC4));
  }

  std::vector<cv::cuda::GpuMat> rgb_mat;
  for (size_t i = 0; i < NUM_CUDA_STREAMS; ++i) {
    rgb_mat.push_back(cv::cuda::GpuMat(args.metadata[0].height,
                                       args.metadata[0].width, CV_8UC3));
  }

  std::vector<cv::cuda::GpuMat> conv_input;
  for (size_t i = 0; i < NUM_CUDA_STREAMS; ++i) {
    conv_input.push_back(cv::cuda::GpuMat(inputHeight, inputWidth, CV_8UC3));
  }
...
void CaffeGPUEvaluator::evaluate(const DatasetItemMetadata& metadata,
                                 char* input_buffer,
                                 std::vector<char*> output_buffers,
                                 int batch_size) {
  size_t frame_size = av_image_get_buffer_size(AV_PIX_FMT_NV12, metadata.width,
                                               metadata.height, 1);
  ...
  for (int i = 0; i < batch_size; ++i) {
    ...
    input_mats[sid] = cv::cuda::GpuMat(metadata.height + metadata.height / 2,
                                       metadata.width, CV_8UC1, buffer);

    convertNV12toRGBA(input_mats[sid], rgba_mat[sid], metadata.width,
                      metadata.height, cv_stream);
    // BGR -> RGB for helnet
    cv::cuda::cvtColor(rgba_mat[sid], rgb_mat[sid], CV_BGRA2RGB, 0, cv_stream);
    ...
    convertRGBInterleavedToPlanar(conv_input[sid], conv_planar_input[sid],
                                  inputWidth, inputHeight, cv_stream);
    conv_planar_input[sid].convertTo(float_conv_input[sid], CV_32FC1,
                                     cv_stream);
    ...
    cv::cuda::divide(normed_input[sid], 255.0f, scaled_input[sid], 1, -1,
                     cv_stream);
    cudaStream_t s = cv::cuda::StreamAccessor::getStream(cv_stream);
    CU_CHECK(
        cudaMemcpy2DAsync(net_input_buffer + i * (inputWidth * inputHeight * 3),
                          inputWidth * sizeof(float), scaled_input[sid].data,
                          scaled_input[sid].step, inputWidth * sizeof(float),
                          inputHeight * 3, cudaMemcpyDeviceToDevice, s));
    ...
    if (false && ((current_frame + i) % 512) == 0) {
      ...
      for (int i = 0; i < rgb_mat[sid].rows; ++i) {
        CU_CHECK(cudaMemcpy(image_buff + metadata.width * 3 * i,
                            rgb_mat[sid].ptr<uint8_t>(i), metadata.width * 3,
  ...
  for (size_t i = 0; i < output_buffer_sizes.size(); ++i) {
    ...
    CU_CHECK(cudaMemcpy(output_buffers[i], output_blob->gpu_data(),
                        batch_size * output_sizes[i], cudaMemcpyDeviceToHost));
  }
}
