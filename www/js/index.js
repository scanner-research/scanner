                       ...
                       videoMetadata)
{
    ...
    $("#threshold-input").change(updatePredictionData);
    ...
    axis.mousemove(function(e) {
        ...
        targetedFrame = Math.floor(targetedFrame / stride) * stride;
        percent = targetedFrame / videoMetadata.frames;

        var index = targetedFrame / stride;

        frameIndicator.text(targetedFrame);
        //frameIndicator.css('left', xPos - labelWidth / 2);
        ...
        if (predictionData.length > 0) {
            var item = predictionData[index];
            jobMetadata.graphics.draw(item);
        }
    });
