...
output_tables = db.run(
    output_op, [job], force=True, pipeline_instances_per_node=2)
...
job = Job(
    op_args={
        ...
        sliced_frame: db.partitioner.all(500),
        gath_hist:
        [db.sampler.gather([0, 15]) for _ in range(num_slice_groups)],
...
# Loop over the column's rows. Each row is a tuple of the frame number and
# value for that row.
video_hists = output_tables[0].column('histogram').load(readers.histograms)
num_rows = 0
for frame_hists in video_hists:
    assert len(frame_hists) == 3
    assert frame_hists[0].shape[0] == 16
