...
def main():
    ...
    video_stream1 = sp.NamedVideoStream(cl, 'example1', path=example_video_path)
    ...
    frames = cl.io.Input([video_stream1])
    ...
    hists = cl.ops.Histogram(frame=frames)
    ...
    output_op = cl.io.Output(hists, [named_stream1])
    ...
    job_id = cl.run(output_op, sp.PerfParams.estimate())
    ...
    named_stream1.delete(cl)
    video_stream2 = sp.NamedVideoStream(cl, 'example2', path=example_video_path)
    frames = cl.io.Input([video_stream1, video_stream2])
    hists = cl.ops.Histogram(frame=frames)
    named_stream2 = sp.NamedStream(cl, 'example2_hist')
    output_op = cl.io.Output(hists, [named_stream1, named_stream2])
